...
def multiclass_nms_c1(
    ...
    labels = torch.arange(num_classes, dtype=torch.long, device=scores.device)
    # labels = torch.zeros(num_classes,dtype=torch.long, device=scores.device)
    labels = labels.view(1, -1).expand_as(scores)
    ...
    with torch.no_grad():
        ...
        embs = model.encode_image(cropped_img).float()
        embs = torch.div(embs,torch.norm(embs,p=2, dim=-1,keepdim=True))
    clip_scores = torch.mm(embs, txt_embs)/0.01
    clip_scores = F.softmax(clip_scores, dim=-1)
    labels = clip_scores.argmax(1)
